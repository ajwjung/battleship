...
const DragDrop = (() => {
  ...
  function markOldSquaresAvailable(ship) {
    const shipName = ship.classList[1];
    const lastOccupiedSquares = document.querySelectorAll(
      `.square.taken.my-${shipName}`
    );
  ...
  function removeTemporaryClasses(ship) {
    ...
    if (ship.classList.contains("rotated-horizontal"))
    ...
    for (let i = 0; i < shipLength; i += 1) {
      ...
      if (orientation === "vertical") {
        ...
        targetSquare = document.getElementById(
          `${lettersKey[lettersKey.indexOf(col) + i]}${row}`
        );
      }
  ...
  function checkSquaresTaken(requiredSquares, ship) {
    ...
    const shipName = ship.classList[1];
    return requiredSquares.some(
      (square) =>
        square.classList.contains("taken") &&
        square.classList[1] !== `my-${shipName}`
    );
  }
  ...
  function isValidDrop(
    ...
    if (orientation === "vertical") {
      validDrop =
        checkOutOfBoundsVertical(squareId, dimension) &&
        !checkSquaresTaken(adjacentSquares, ship);
    } else if (orientation === "horizontal") {
      validDrop =
        checkOutOfBoundsHorizontal(squareId, dimension) &&
        !checkSquaresTaken(adjacentSquares, ship);
    }
  ...
  function shipNeverRotatedBefore(ship) {
    return (
      ...
      !ship.classList.contains("rotated-horizontal")
    );
  }
  ...
  function dragDrop(e) {
    ...
    if (e.target.classList.contains("square")) {
      ...
      if (
        shipNeverRotatedBefore(ship) &&
        firstDropForShip(ship, shipWidth) &&
        validDrop
      ) {
        ...
        ship.classList.contains("rotated-vertical") &&
        firstDropForShip(ship, shipHeight) &&
        validDrop
      ) {
      ...
      if (
        (hasVerticalClass(ship) || hasHorizontalClass(ship)) &&
        ship.parentNode.classList.contains("square") &&
        validDrop
      ) {
        markOldSquaresAvailable(ship);
        e.target.append(ship);
        adjacentSquares.forEach((newSquare) =>
          markSquareTaken(ship, newSquare)
        );
        removeTemporaryClasses(ship);
      }
    }
