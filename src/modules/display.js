const Display = (() => {
  ...
  function opponentResponse(wasHit, hitTarget, opponent) {
    const message =
      opponent.playerName !== "computer" ? playerMsgBox : cpuMsgBox;
  ...
  function updatePeg(coordinates, wasHit, opponent) {
    const formattedCoordinates = `${coordinates[1]}${coordinates[0]}`;
    const targetSquarePeg =
      opponent.playerName !== "computer"
  ...
  function addStartGameText() {
    ...
    infoText.textContent =
      "The first to sink all 5 of their opponent's ships wins. Good luck!";
  ...
  function resetCpuShipDock() {
    const leftShipDock = document.querySelector(
      "#opponent-ships > .left-ships"
    );
    const rightShipDock = document.querySelector(
      "#opponent-ships > .right-ships"
    );
  ...
  function resetGameGrids() {
    ...
    const cpuGridSquares = document.querySelectorAll(
      "#opponent-board .square:not(.legend)"
    );
